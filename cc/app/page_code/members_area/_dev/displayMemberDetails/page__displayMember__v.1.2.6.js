...
async function loadFullData(memberId) {
   try {
       // 1. Updated collection name to ccFullData
       const result = await wixData.query('ccFullData')
           .eq('memberId', memberId) // Verify this is the 'Field Key', not the Label
           .limit(1)
           .find();
       ...
       if (result.items.length > 0) {
           ...
           // 2. Map data using exact Field Keys (Standard 6.2)
           ...
           safeSetText('#zipCode', data.zipCode || "No Zip Code On File");
          
           // 3. Handle Website Link (Standard 4.3)
           const website = data.customWebsiteUrl || '';
           ...
           // 4. Handle Cover Photo (Fixes "src" SDK Error)
           const coverEl = $w('#coverPhoto');
           if (coverEl && typeof coverEl.src !== 'undefined') {
               // Validate that it's a Wix URL or HTTP link before setting
               const isValidUrl = data.coverPhoto && (data.coverPhoto.startsWith('wix:image') || data.coverPhoto.startsWith('http'));
               coverEl.src = isValidUrl ? data.coverPhoto : DEFAULT_COVER_PHOTO;
           }
       } else {
           console.warn(`${VERSION} No record found in ccFullData for: ${memberId}`);
...
/**
* Standard 4.4: Fixes SDK error for profile photo
*/
function renderMemberDetails(member) {
   try {
       safeSetText('#firstName', member.contactDetails?.firstName);
       ...
       const photoEl = $w('#profilePhoto');
       if (photoEl && typeof photoEl.src !== 'undefined') {
           const url = member.profile?.photo?.url;
           // Only set src if it's a valid URL string
           photoEl.src = (url && url.length > 5) ? url : DEFAULT_PROFILE_PHOTO;
       }
   } catch (err) {
       console.error(`${VERSION} Render Error:`, err);
   }
