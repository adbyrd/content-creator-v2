...
$w.onReady(async function () {
    console.log('[cc-v2.0.3] Initializing Profile Page...'); // Standardized logging 
    ...
    try {
        ...
        if (member) {
            ...
            console.warn('[cc-v2.0.3] hydrateProfile: No active session.');
            showError(MSG_NOT_LOGGED_IN);
        }
    } catch (err) {
        // Defensive logging for global failures
        console.error('[cc-v2.0.3] Global initialization failure:', err);
        showError(MSG_LOAD_ERROR);
    }
});

/**
 * Maps the member object to UI elements with strict fallback logic
 */
function hydrateProfile(member) {
    ...
    // --- 1. Core Identity & Formatting Fixes ---
    ...
    // --- 2. Date Fix: Strict Check for toLocaleDateString ---
    // This prevents the TypeError seen in browser.log 
    let formattedDate = null;
    if (lastLoginDate instanceof Date && !isNaN(lastLoginDate)) {
...
/**
 * Safe UI Helpers (Defensive Programming) 
 */
function safeSetText(selector, value) {
    ...
    if (el && typeof el.text !== 'undefined') {
        ...
        console.warn(`[cc-v2.0.3] Element ${selector} not found on page.`);
    }
}
