...
import { updateMemberProfile } from 'backend/ccMembers.web'; // .jsw depricated.
...
async function handleSubmit() {
    ...
    try {
        // 1. Identify the current member
        const member = await currentMember.getMember();
        if (!member) throw new Error("No member logged in.");

        // 2. Call the backend wrapper [Refactored Line]
        await updateMemberProfile(member._id, payload);

        // 3. Backup to collection (existing logic)
