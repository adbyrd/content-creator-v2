/**
 * MEMBER DATA SERVICE
 * Handles CRUD operations for the 'ccProfiles' collection.
...
const MEMBERS_COLLECTION = 'ccProfiles';
...
export const getMemberBusinessProfile = webMethod(Permissions.Anyone, async () => {
    ...
    console.log(`[cc-v2.2.3][${requestId}] Initiating profile retrieval...`);

    try {
        // 1. Authentication Check
        const member = await currentMember.getMember();
        if (!member) {
            console.warn(`[cc-v2.2.3][${requestId}] Retrieval blocked: No member session.`);
            return { 
                ok: false, 
                status: 401, 
        ...
        if (result.items.length > 0) {
            console.log(`[cc-v2.2.3][${requestId}] Profile found for member: ${member._id}`);
            return { 
                ok: true, 
                status: 200, 
                data: result.items[0] 
        ...
        console.warn(`[cc-v2.2.3][${requestId}] System state error: Profile record missing for ${member._id}`);
        return { 
            ok: false, 
            status: 404, 
        ...
        console.error(`[cc-v2.2.3][${requestId}] Critical Database Error:`, error);
        return { 
            ok: false, 
            status: 500, 
...
export const saveMemberBusinessProfile = webMethod(Permissions.Anyone, async (data) => {
    ...
    console.log(`[cc-v2.2.3][${requestId}] Persistence request received.`);
    ...
    try {
        ...
        if (queryResult.items.length > 0) {
            ...
            console.log(`[cc-v2.2.3][${requestId}] Merging existing record for ${member._id}`);
        } else {
            // INITIAL CREATION
            recordToSave = { 
                ...data, 
            ...
            console.log(`[cc-v2.2.3][${requestId}] Creating initial record for ${member._id}`);
        }
        ...
        console.error(`[cc-v2.2.3][${requestId}] Persistence Failure:`, error);
        return { 
            ok: false, 
            status: 500, 
